...
BddNodeV
BddMgrV::restrict(const BddNodeV& f, const BddNodeV& g)
{
   if(g == BddNodeV::_zero) return size_t(0);
   if(g == BddNodeV::_one) return f;
   if(f == BddNodeV::_one || f == BddNodeV::_zero) return f;
   ...
   BddNodeV node_a = getSupport(a);
   return (~node_a & restrict(f.getRightCofactor(a), g.getRightCofactor(a))) | (node_a & restrict(f.getLeftCofactor(a), g.getLeftCofactor(a)));
   // return 
